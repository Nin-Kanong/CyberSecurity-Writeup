...
![hydra-contrasenas](https://github.com/user-attachments/assets/4a11975d-65da-4594-a7e7-c3cab88bc258)
...
- `-V`: Enables verbose mode, showing each login attempt in real time.
- This command is ideal for:
  • 	Credential brute-forcing in penetration testing labs
  • 	Validating SSH exposure and password strength
...
- This command is ideal for:
  • 	Credential brute-forcing in penetration testing
  • 	Validating SSH exposure and weak authentication
...
- `-vV`: Enables verbose and very verbose output, showing each login attempt.
- `-f`: Stops after the first valid credential pair is found.
- This command is ideal for:
  • 	Efficient brute-force testing in penetration testing.
  • 	Quickly identifying valid SSH credentials
...
- `-e nsr`: Tries null, same-as-login, and reverse login passwords
- `-u`: Ensures each username is tried fully before moving to the next (useful for slow or rate-limited services).
- This command is ideal for:
  - Testing weak or default SSH credentials
  - Exploring common password patterns (null, same, reverse)
...
````
hydra -l msfadmin -x 1:2:aA1 ftp://192.168.127.129 -V
````
<img width="995" height="782" alt="image" src="https://github.com/user-attachments/assets/86e950a3-6c9c-41df-8529-e91e3798960f" />

- `-x 1:2:aA1`: Dynamically generates passwords with:
  - Length from 1 to 2 characters.
  - - Character set: lowercase (a), uppercase (A), digits (1)   ftp://192.168.127.129  Specifies the target IP and protocol (FTP)   -V  Enables verbose mode, showing each login attempt in real time 
- This command is ideal for:
  • 	Credential brute-forcing in penetration testing labs (e.g., Metasploitable)
  • 	Validating FTP exposure and weak authentication


- We can use this to generate from a -> b ... 

To generate passwords using various set of characters, you can use -x option. It is used as 
x  min:max:charset  where, 
...
- `http-post-form`: Specifies a form-based HTTP POST login.
- `"/dvwa/login.php:username=^USER^&password=^PASS^&Login=Login:F=Login failed"`:
  - Form path: `/dvwa/login.php`
  - POST data: `username`, `password`, and `Login` button.
  - Failure string: `"Login failed"` used to detect unsuccessful attempts   `-V`  Enables verbose mode, showing each login attempt in real time
 
- This command is ideal for:
  - Testing weak web login credentials in DVWA or similar lab environments
  - Validating form-based authentication exposure
...
- To check `Method`:
<img width="1310" height="865" alt="image" src="https://github.com/user-attachments/assets/807fc378-52c3-4209-8517-fe4ce9fe96aa" />
...
### Brute Force others Website 
- In this i want to brute force with `OWASP Mutillidae`: http://192.168.100.97:5000/index.php?page=login.php
<img width="1855" height="972" alt="image" src="https://github.com/user-attachments/assets/a8db6bfc-297b-4031-9950-f5bf970a36c4" />

- If in this i `Login` with wrong Username and Password:
````
Username: admin
Pasword: admin
````
<img width="1658" height="708" alt="image" src="https://github.com/user-attachments/assets/7833b946-f793-4696-96b6-d37da8d6a043" />

<img width="1857" height="959" alt="image" src="https://github.com/user-attachments/assets/2bfef5dd-9b2f-42be-a234-4daf64052c80" />

- After Press `F12` and find this:
<img width="1847" height="973" alt="image" src="https://github.com/user-attachments/assets/7c8d4f6f-e12c-497a-8ce8-ac3f868d8500" />

- So in this we found the `Method=post`:

<img width="1855" height="852" alt="image" src="https://github.com/user-attachments/assets/8342dfb8-f085-4f3c-8401-2299c30f40f3" />

````
hydra -l admin -P /usr/share/wordlists/passwd.txt -s 5000 192.168.100.97 http-post-form "/index.php:page=login.php&username=^USER^&password=^PASS^&login-php-submit-button=Login:F=Password incorrect" -f -V
````
<img width="1902" height="917" alt="image" src="https://github.com/user-attachments/assets/2b890758-ff0d-458e-b212-3cc836d548a6" />

- So this i found the correct **Username**: `admin` and **Password**: `admin`.
- If in this i try to login:
<img width="1852" height="912" alt="image" src="https://github.com/user-attachments/assets/c158dabb-1674-4fbf-9fbb-64cc91c05700" />

<img width="1858" height="951" alt="image" src="https://github.com/user-attachments/assets/e8eb91ef-1cea-44a6-b471-1e9c8559b222" />
...
- `-vv`: Enables double verbose mode, showing detailed output for each attempt.
- `-s 3306`:  Specifies the MySQL port (default is 3306)
- `-e nsr`: Tries null, same-as-login, and reverse login passwords in addition to the wordlist.
- This command is ideal for:
  • 	Credential brute-forcing against MySQL in penetration testing labs
  • 	Validating database exposure and authentication strength
